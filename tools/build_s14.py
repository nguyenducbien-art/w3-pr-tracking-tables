...
var TEST_S={done:['pill-approved','✅ xong'],wip:['pill-changes','🔵 đang'],todo:['pill-todo','⚪ chưa']};
function testCell(v){if(!v)return '<span class="cf-na">—</span>';var x=TEST_S[v]||TEST_S.todo;return '<span class="pill '+x[0]+'">'+x[1]+'</span>';}
function renderScreens(S){
  ...
  function nt(v){return scr.filter(function(s){return s.test===v;}).length;}
  var hasTest=scr.some(function(s){return s.test;});
  ...
  var body=scr.map(function(s){
    ...
    return '<tr data-pic="'+esc(s.pic)+'"><td><span class="ticket">'+s.sid+'</span></td>'
      ...
      +'<td>'+(s.tester?'<span class="dev">'+esc(s.tester)+'</span>':'<span class="cf-na">—</span>')+'</td>'
      +'<td><span class="note-cell">'+esc(s.note||'')+'</span></td></tr>';
  }).join('');
  var _PF=filterBlock(scr,'pic');
  var html='<div class="page">'
    +'<div class="page-header"><h1>Danh sách màn — Sprint 14 (Kho / Inventory)</h1>'
    ...
    +'<div class="stats">'
      ...
      +(hasTest?('<div class="stat" style="margin-left:8px;border-left:1px solid var(--border);padding-left:16px">Test tích hợp ✅ <span class="stat-val" style="color:var(--report-fg)">'+nt('done')+'</span></div>'
        +'<div class="stat">🔵 đang <span class="stat-val" style="color:var(--pill-changes-fg)">'+nt('wip')+'</span></div>'
        +'<div class="stat">⚪ chưa <span class="stat-val warn">'+nt('todo')+'</span></div>'):'')
    +'</div>'
    +'<div class="pic-summary">'+chips+'</div>'
    +_PF.bar+'<div class="scroll-wrap"><table style="min-width:1040px"><thead><tr>'
      +'<th>Screen ID</th><th>Tên màn</th><th>Route Next.js</th><th>Ticket</th><th>PIC</th><th>Migrate</th><th>Test tích hợp</th><th>Người test</th><th>Ghi chú</th>'
    +'</tr></thead><tbody'+(_PF.id?' id="'+_PF.id+'"':'')+'>'+body+'</tbody></table></div>'
    +'<div class="footnote">✅ migrated = có folder màn (khai báo <code>SCREEN_ID</code>) trên r20260727 · 🔴 chưa = chưa có folder (đang làm ở branch riêng hoặc chưa bắt đầu).<br>'
      +'Test tích hợp (integration test): ✅ xong = Resolved/Closed · 🔵 đang = In Progress · ⚪ chưa = Open — lấy từ task <b>テスト実施</b> (test execution, milestone Sprint 14) trên Backlog; Người test = assignee của task đó.<br>'
      +'Danh sách màn = milestone “Sprint 14” trên Backlog (category 画面実装 = screen impl). Cron điều tra r20260727 + Backlog mỗi 5 phút; ticket = issueKey Backlog. Reassign PIC / thêm-bớt màn: sửa CONFIG trong <code>tools/fetch_screens_s14.py</code>.</div>'
    +'</div>';
