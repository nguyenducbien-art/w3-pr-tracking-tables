...
#   5) fetch_routes_s15.py   → routes-s15.json   (Sprint 15 route React per-sprint — git nhánh per-sprint)
# Đẩy CẢ file vào nhánh `data` bằng git plumbing → KHÔNG checkout, KHÔNG build Pages
...
cd "$REPO" || exit 1
{
  ...
  for pair in "s13:$rc13" "s12:$rc12" "s14:$rcPR" "s15:$rc15" "plan15:$rcPl" "routes15:$rcRt" "screens:$rcSc"; do
    n=${pair%:*}; rc=${pair#*:}
    if [ "$rc" -ne 0 ] && [ "$rc" -ne 2 ]; then echo "→ LỖI fetch $n (rc=$rc)"; exit 1; fi
  done
  if [ "$rc13" -eq 2 ] && [ "$rc12" -eq 2 ] && [ "$rcPR" -eq 2 ] && [ "$rc15" -eq 2 ] && [ "$rcPl" -eq 2 ] && [ "$rcRt" -eq 2 ] && [ "$rcSc" -eq 2 ]; then
